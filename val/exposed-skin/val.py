...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="使用 exposed-skin 模型对图片目录进行验证，并输出标准化类别结果。"
    )
    parser.add_argument(
        "--images",
        type=Path,
        default=Path("val/exposed-skin/images"),
        help="待检测图片目录，默认 val/exposed-skin/images",
    )
    parser.add_argument(
        "--model",
        type=Path,
        default=Path("model/exposed-skin/best.pt"),
        help="模型权重路径，默认 model/exposed-skin/best.pt",
    )
    parser.add_argument(
        "--yaml",
        type=Path,
        default=Path("exposed-skin.yaml"),
        help="类别配置 YAML，默认 exposed-skin.yaml",
    )
    parser.add_argument(
        "--conf",
        type=float,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("val/exposed-skin/predictions.json"),
        help="输出 JSON 文件路径，默认 val/exposed-skin/predictions.json",
    )
    parser.add_argument(
        "--save-vis",
        action="store_true",
        help="是否保存可视化结果到 val/exposed-skin/annotated",
    )
...
def main() -> None:
    ...
    vis_dir = Path("val/exposed-skin/annotated")
    if args.save_vis:
        if vis_dir.exists():
