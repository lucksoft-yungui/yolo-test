...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="播放视频并用训练好的实验服模型绘制目标框")
    parser.add_argument(
        "--video",
        type=Path,
        default=Path("videos") / "lab coat.mp4",
        help="源视频路径，默认 videos/lab coat.mp4",
    )
    parser.add_argument(
        "--model",
        type=Path,
        default=Path("model/labcoat/weights/best.pt"),
        help="训练好的模型权重，默认 model/labcoat/weights/best.pt",
    )
    parser.add_argument(
        "--conf",
        type=float,
