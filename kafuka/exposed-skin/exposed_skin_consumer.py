...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="订阅主题并检测 实验服 + 手套 + 皮肤裸露"
    )
    parser.add_argument(
        "--bootstrap-servers",
        type=str,
    ...
    parser.add_argument(
        "--topic",
        type=str,
        default="exposed-skin-alarm",
        help="Kafka 主题，默认 exposed-skin-alarm",
    )
    parser.add_argument(
        "--alarm-topic",
        type=str,
        default="exposed-skin-alarm-result",
        help="报警消息推送主题，默认 exposed-skin-alarm-result",
    )
    parser.add_argument(
        "--group-id",
        type=str,
        default="exposed-skin-alarm-consumer",
        help="消费者组 ID",
    )
    parser.add_argument(
        "--batch-size",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            os.getenv("EXPOSED_SKIN_MODEL_PATH", "model/exposed-skin/best.pt")
        ),
        help="皮肤裸露模型权重路径",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--exposed-skin-yaml",
        type=Path,
        default=Path(os.getenv("EXPOSED_SKIN_YAML", "exposed-skin.yaml")),
        help="皮肤裸露数据集 yaml",
    )
    parser.add_argument(
        "--labcoat-class-name",
        ...
        help='穿实验服的类别名称，默认 "with labcoat"',
    )
    parser.add_argument(
        "--glove-class-name",
        type=str,
        default="with glove",
        help='戴手套类别名称，默认 "with glove"',
    )
    parser.add_argument(
        "--exposed-skin-class-name",
        type=str,
        default="exposed-skin",
        help='皮肤裸露类别名称，默认 "exposed-skin"',
    )
    parser.add_argument(
        "--labcoat-class-id",
        type=int,
        default=None,
        help="穿实验服类别索引（不填则按名称解析）",
    )
    parser.add_argument(
        "--glove-class-id",
        type=int,
        default=None,
        help="戴手套类别索引（不填则按名称解析）",
    )
    parser.add_argument(
        "--exposed-skin-class-id",
        type=int,
        default=None,
        help="皮肤裸露类别索引（不填则按名称解析）",
    )
    parser.add_argument(
        "--labcoat-conf",
        type=float,
    ...
    parser.add_argument(
        "--debug-dir",
        type=Path,
        default=Path("kafuka/exposed-skin/debug"),
        help="调试图保存目录，默认 kafuka/exposed-skin/debug",
    )
...
def draw_boxes(image, boxes: list[dict[str, object]]) -> object:
    for box in boxes:
        ...
        color = (0, 200, 0)
        if class_name == "with glove":
            color = (255, 165, 0)
        elif class_name == "exposed-skin":
            color = (0, 0, 255)
...
def clip_box(coords: list[float], width: int, height: int) -> tuple[int, int, int, int]:
    x1, y1, x2, y2 = [int(v) for v in coords]
...
def main() -> None:
    ...
    exposed_skin_model = load_model(
        args.exposed_skin_model, device, exposed_skin_names
    )
    ...
    print(
        ...
        f"皮肤裸露模型 {args.exposed_skin_model}，group={args.group_id}，"
        f"offset={args.auto_offset_reset}",
        flush=True,
    )
    ...
    while True:
        ...
        labcoat_boxes_per_entry: list[list[dict[str, object]]] = []
        crop_images: list[object] = []
        crop_mapping: list[tuple[int, int, int, int]] = []

        for entry_idx, (result, img) in enumerate(zip(labcoat_results, images)):
            ...
            if result.boxes is not None:
                for box in result.boxes:
                    ...
                    boxes_for_entry.append(
                        {
                            "className": labcoat_names.get(with_labcoat_id, "with labcoat"),
                            "conf": conf,
                            "xyxy": coords,
                        }
                    )

                    x1, y1, x2, y2 = clip_box(coords, img.shape[1], img.shape[0])
                    if x2 <= x1 or y2 <= y1:
                        continue
                    crop = img[y1:y2, x1:x2]
                    crop_images.append(crop)
                    crop_mapping.append((entry_idx, len(boxes_for_entry) - 1, x1, y1))
            labcoat_boxes_per_entry.append(boxes_for_entry)

        glove_boxes_per_entry: list[list[dict[str, object]]] = [[] for _ in valid_entries]
        exposed_skin_boxes_per_entry: list[list[dict[str, object]]] = [
            [] for _ in valid_entries
        ]
        matched_labcoat_indices: list[set[int]] = [set() for _ in valid_entries]

        if crop_images:
            glove_results = glove_model(crop_images, conf=args.glove_conf, verbose=False)
            exposed_skin_results = exposed_skin_model(
                crop_images, conf=args.exposed_skin_conf, verbose=False
            )

            for mapping, glove_result, skin_result in zip(
                crop_mapping, glove_results, exposed_skin_results
            ):
                entry_idx, labcoat_box_idx, x_offset, y_offset = mapping

                glove_hits: list[dict[str, object]] = []
                if glove_result.boxes is not None:
                    for box in glove_result.boxes:
                        cls_id = int(box.cls[0])
                ...
                skin_hits: list[dict[str, object]] = []
                if skin_result.boxes is not None:
                    for box in skin_result.boxes:
                        cls_id = int(box.cls[0])
                        if cls_id != exposed_skin_id:
                            continue
                        conf = float(box.conf[0]) if hasattr(box, "conf") else None
                        sx1, sy1, sx2, sy2 = [float(v) for v in box.xyxy[0]]
                        skin_hits.append(
                            {
                                "className": exposed_skin_names.get(
                                    exposed_skin_id, "exposed-skin"
                                ),
                                "conf": conf,
                                "xyxy": [
                                    sx1 + x_offset,
                ...
                if glove_hits and skin_hits:
                    matched_labcoat_indices[entry_idx].add(labcoat_box_idx)
                    glove_boxes_per_entry[entry_idx].extend(glove_hits)
                    exposed_skin_boxes_per_entry[entry_idx].extend(skin_hits)

        inference_elapsed = time.monotonic() - inference_start

        hit_count = 0
        for entry_idx, entry in enumerate(valid_entries):
            labcoat_boxes = labcoat_boxes_per_entry[entry_idx]
            glove_boxes = glove_boxes_per_entry[entry_idx]
            exposed_skin_boxes = exposed_skin_boxes_per_entry[entry_idx]

            if not labcoat_boxes or not glove_boxes or not exposed_skin_boxes:
                continue
            ...
            matched_labcoat_boxes = [
                box for idx, box in enumerate(labcoat_boxes) if idx in matched_indices
            ]
            if not matched_labcoat_boxes:
                continue
            ...
            merged_boxes.extend(matched_labcoat_boxes)
            merged_boxes.extend(glove_boxes)
            merged_boxes.extend(exposed_skin_boxes)
            ...
            print(
                ...
                f"with_labcoat={len(matched_labcoat_boxes)}, glove={len(glove_boxes)}, "
                f"exposed_skin={len(exposed_skin_boxes)}",
                flush=True,
            )
