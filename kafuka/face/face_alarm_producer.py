...
def parse_args() -> argparse.Namespace:
    default_dir = Path("/Users/peiyandong/Documents/code/ai/yolo-test/datasets/labcoat/images/train")
    parser = argparse.ArgumentParser(description="手动推送人脸校验图片到 Kafka")
    parser.add_argument(
        "--bootstrap-servers",
        type=str,
    ...
    parser.add_argument(
        "--topic",
        type=str,
        default="face_recognition_alarm",
        help="Kafka 主题，默认 face_recognition_alarm",
    )
    parser.add_argument(
        "--image",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=default_dir,
        help=f"从目录递归收集 .jpg 图片，默认 {default_dir}",
    )
    parser.add_argument(
        "--limit",
        type=int,
