...
## 运行示例
```bash
uv run python kafuka/fire/alarm_consumer.py \
  ...
  --conf 0.8 \
  --imgsz 1280 \
  --target-class-index 0,1 \
  --target-class-name fire,smoke \
  --debug
...
uv run python kafuka/fire/fire_alarm_producer.py \
  --bootstrap-servers localhost:9092 \
  --topic fire-alarm \
  --image-dir kafuka/fire/images \
  --limit 100
